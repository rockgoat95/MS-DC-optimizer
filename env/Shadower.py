...
class shadowerEnv(gym.Env):
    ...
    def get_state(self):
        ...
        ability = [
            self.main_stat / 50000,
            self.sub_stat / 10000,
            self.critical_damage / 100,
            (self.damage + self.boss_damage) / 300,
            self.att_p / 100,
            self.defense_ignore / 100,
            self.final_damage / 100,
        ]
        activation_time = [
            self.ready_to_die_activation_time / (60 * self.FRAME),
        ...
        self.state = (
            [self.current_time / self.dealing_time]
    ...
    def step(self, action):
        ...
        if self.spyder_in_mirror_activation_time > 0:
            ...
            spyder_in_mirror_att_frame = [
                frame * self.FRAME for frame in spyder_in_mirror_att_frame
            ]
        ...
        if self.vail_of_shadow_activation_time > 0:
            ...
            if self.vail_of_shadow_activation_time in vail_of_shadow_att_frame:
                ability_dict = {
                    ...
                    "att_p": self.att_p,
                    "boss_damage": self.boss_damage,
                    "defense_ignore": self.defense_ignore_calculator(
                        [self.defense_ignore, 20]
                    ),
                    "final_damage": self.final_damage,
        ...
        if self.ready_to_die_activation_time == 1:
            self.final_damage = self.final_damage_applier(
                self.final_damage, 36, activation=False
            )

        if self.ultimate_dark_sight_activation_time == 1:
            self.final_damage = self.final_damage_applier(
                self.final_damage, 31, activation=False
            )
            if (self.vail_of_shadow_activation_time > 1) or (
                self.smoke_shell_activation_time > 1
            ):
                self.final_damage = self.final_damage_applier(
        ...
        if self.vail_of_shadow_activation_time == 1:
            if (self.ultimate_dark_sight_activation_time > 1) or (
                self.smoke_shell_activation_time > 1
            ):
                pass
            else:
        ...
        if self.smoke_shell_activation_time == 1:
            if (self.vail_of_shadow_activation_time > 1) or (
                self.ultimate_dark_sight_activation_time > 1
            ):
                pass
            else:
                self.final_damage = self.final_damage_applier(
        ...
        self.ready_to_die_activation_time = max(
            0, self.ready_to_die_activation_time - 1
        )
        self.ultimate_dark_sight_activation_time = max(
            0, self.ultimate_dark_sight_activation_time - 1
        )
        self.maple_world_goddess_blessing_activation_time = max(
            0, self.maple_world_goddess_blessing_activation_time - 1
        )
        self.spyder_in_mirror_activation_time = max(
            0, self.spyder_in_mirror_activation_time - 1
        )

        self.soul_contract_activation_time = max(
            0, self.soul_contract_activation_time - 1
        )
        self.restraint_ring_activation_time = max(
            0, self.restraint_ring_activation_time - 1
        )
        self.weaponpuff_ring_activation_time = max(
            0, self.weaponpuff_ring_activation_time - 1
        )
        self.vail_of_shadow_activation_time = max(
            0, self.vail_of_shadow_activation_time - 1
        )
        self.smoke_shell_activation_time = max(0, self.smoke_shell_activation_time - 1)
        self.epic_adventure_activation_time = max(
            0, self.epic_adventure_activation_time - 1
        )

        self.sonic_blow_cool_time = max(self.sonic_blow_cool_time - 1, 0)
        self.slash_shadow_formation_cool_time = max(
            self.slash_shadow_formation_cool_time - 1, 0
        )
        self.incision_cool_time = max(self.incision_cool_time - 1, 0)
        self.ultimate_dark_sight_cool_time = max(
            self.ultimate_dark_sight_cool_time - 1, 0
        )
        self.ready_to_die_cool_time = max(self.ready_to_die_cool_time - 1, 0)
        self.maple_world_goddess_blessing_cool_time = max(
            self.maple_world_goddess_blessing_cool_time - 1, 0
        )
    ...
    def slash_shadow_formation(self):
        ...
        self.slash_shadow_formation_cool_time = self.cool_time_modifier(
            90 * self.FRAME, 5
        )
        return None
    ...
    def soul_contract(self):
        ...
        self.soul_contract_activation_time = self.buff_time_modifier(
    ...
    def maple_world_goddess_blessing(self):  # 수치조정
        ...
        self.maple_world_goddess_blessing_activation_time = 60 * self.FRAME
        self.maple_world_goddess_blessing_cool_time = self.cool_time_modifier(
            180 * self.FRAME, 5
        )
        self.damage = self.damage + 20
        self.main_stat = self.main_stat + 4265
    ...
    def ultimate_dark_sight(self):  # 0.9 초 준비 1.6초동안 순수 7타 1.6
        ...
        self.ultimate_dark_sight_cool_time = self.cool_time_modifier(
            190 * self.FRAME, 5
        )
    ...
    def null_action(self):
        ability_dict = {
            ...
            "att_p": self.att_p,
            "boss_damage": self.boss_damage + 35,
            "defense_ignore": self.defense_ignore_calculator(
                [self.defense_ignore, 20, 10]
            ),
        ...
        self.step_reward += (
            0.5 * 6 * line_damage / int(self.att_skill_delay(1230) / 1000 * self.FRAME)
        )  # 소닉블로우에 대한 암암메의 프레임당 기대데미지
    ...
    def skill_damage_calculator(self, ability_dict):
        ...
        skill_damage = (
            ((main_stat * 4 + sub_stat) * 2132 * 1.3 * 1 / 100)
            * (skill_damage / 100)
            * critical_constant
            * ((100 + att_p) / 100)
            * ((100 + damage + boss_damage) / 100)
